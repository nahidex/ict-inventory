...
describe('NOC Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    // Setup Setup
    const cat = await prisma.category.create({ data: { name: 'IT', code: 'IT' } });
    const officer = await prisma.officer.create({ data: { name: 'Jane NOC' } });
    officerId = officer.id;
    const asset = await (prisma.asset as any).create({ 
      data: { assetTag: 'NOC-AST-01', categoryId: cat.id, status: 'Available' } 
    });
    assetId = asset.id;
  ...
  describe('Clearance Check & NOC Process', () => {
    ...
    it('should show officer NOT clear after assignment', async () => {
      ...
      await (prisma.asset as any).update({ where: { id: assetId }, data: { status: 'Assigned' } });
    ...
    it('should allow NOC application after returning assets', async () => {
      // Return asset
      const assignment = await prisma.assignment.findFirst({ where: { officerId, actualReturnDate: null } });
      await (prisma.assignment as any).update({
        where: { id: assignment!.id },
        data: { actualReturnDate: new Date() }
      });
      await (prisma.asset as any).update({ where: { id: assetId }, data: { status: 'Available' } });
