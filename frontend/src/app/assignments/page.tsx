          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low">
                <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">এসেট তথ্য</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">অফিসার</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">ইস্যুর তারিখ</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">রিটার্ন তারিখ</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider">অবস্থা</th>
                <th className="px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider text-right">অ্যাকশন</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-on-surface text-sm">{item.asset.assetTag}</div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-primary-container flex items-center justify-center text-[10px] font-black text-primary border border-primary/10 overflow-hidden">
                      ...
                      <div>
                        <div className="text-sm font-bold text-on-surface">{item.officer.name}</div>
                        <div className="text-[10px] text-on-surface-variant font-medium">{item.officer.designation}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-on-surface">{formatDate(item.issueDate)}</div>
                    <div className="text-[10px] text-on-surface-variant uppercase tracking-wider">ইস্যু করেছেন: {item.issuedBy || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold ring-1 ring-inset ${
                      !item.actualReturnDate 
                        ? 'bg-amber-100 text-amber-800 ring-amber-600/20' 
                        : 'bg-green-100 text-green-800 ring-green-600/20'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${!item.actualReturnDate ? 'bg-amber-600' : 'bg-green-600'}`}></span>
                  ...
                  <td className="px-6 py-4 text-right">
                    {!item.actualReturnDate ? (
                      <button 
                        ...
                        className="text-xs font-bold text-primary hover:bg-primary/10 px-4 py-2 rounded-full transition-colors flex items-center gap-1 ml-auto"
                      >
                        <MaterialIcon name="assignment_return" size={16} />
                        ফেরত নিন
                      </button>
                    ) : (
                      <button className="p-2 hover:bg-surface-container-high rounded-full text-on-surface-variant transition-colors" title="বিস্তারিত">
