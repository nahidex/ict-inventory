...
export default function TopBar() {
  ...
  return (
    <header 
      className="fixed top-16 lg:top-0 right-0 h-16 bg-surface/90 backdrop-blur-md border-b border-outline-variant z-40 px-4 lg:px-8 flex justify-between items-center transition-all duration-300"
      style={{ left: typeof window !== 'undefined' && window.innerWidth < 1024 ? '0' : (isCollapsed ? '80px' : '280px') }}
    >
      <div className="flex items-center gap-4 flex-1 lg:flex-none">
        <h2 className="text-lg lg:text-xl font-bold text-primary tracking-tight font-sans whitespace-nowrap">অ্যাসেটফ্লো</h2>
      </div>

      <div className="hidden md:block flex-[2] max-w-2xl px-4">
      ...
      <div className="flex items-center gap-2 lg:gap-4 flex-1 justify-end">
        <button 
          ...
          className="bg-primary text-on-primary p-2 lg:px-4 lg:py-2 rounded-full text-sm font-bold flex items-center gap-2 hover:brightness-110 transition-all shadow-md shadow-primary/20"
        >
          <MaterialIcon name="add" size={20} />
          <span className="hidden lg:inline">নতুন এসেট</span>
        </button>
        <div className="hidden lg:block h-8 w-[1px] bg-outline-variant mx-1"></div>
        ...
        <div className="h-8 w-8 lg:h-10 lg:w-10 rounded-full bg-primary-container overflow-hidden border border-outline-variant cursor-pointer hover:ring-2 hover:ring-primary/20 transition-all">
          <img 
